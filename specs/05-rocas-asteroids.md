# SPEC 05 — ROCAS: asteroides jugable

> **Estado:** Aprobado  
> **Depende de:** SPEC 01  
> **Fecha:** 2026-09-21  
> **Objetivo:** Adaptar el Asteroids de `references/started-games/02-asteroids` a Next.js para que `/games/rocas/play` sea jugable con canvas, power-ups y guardado de puntuación de la plataforma.
...
- Mientras el modal está abierto: **Espacio no reinicia**; el reinicio es vía botón **JUGAR DE NUEVO** (añadirlo al modal) o volviendo a entrar a `/games/rocas/play`
- Botones del player en ROCAS:
  - **Pausa / Reanudar** — pausa o reanuda el loop del juego
  - **Fin** — fuerza `gameover` y abre el modal
  - **Salir** — navega a `/games/rocas` (detalle)
- Condicionar en `app/games/[id]/play/page.tsx`: si `id === 'rocas'`, montar `AsteroidsGame` en lugar del placeholder CRT; el resto de juegos sigue con el placeholder y la simulación de score actual
...
- Cambiar metadatos de `GAMES` (id `rocas`, copy, cover) salvo lo imprescindible para cablear el play
...
Esta feature no modifica el tipo `Game` ni el array `GAMES` más allá de usar el id existente `rocas`.
...
4. **Cablear `/games/rocas/play`** — En `app/games/[id]/play/page.tsx`, si `id === 'rocas'`: renderizar `AsteroidsGame` dentro de `.crt-screen` (sin siluetas placeholder); desactivar el intervalo de score falso; sincronizar score (y opcionalmente vidas/nivel) desde callbacks hacia el HUD de la plataforma. Otros `id` sin cambios.
...
- [ ] `/games/rocas/play` muestra un canvas 800×600 jugable (no el placeholder de nave/enemigos CSS)
...
- [ ] **GUARDAR** escribe en `localStorage` bajo `av_scores` con `game: 'rocas'`
- [ ] **JUGAR DE NUEVO** inicia una partida nueva en la misma ruta
- [ ] **Salir** navega a `/games/rocas`
- [ ] Cualquier otro `/games/[id]/play` distinto de `rocas` conserva el comportamiento placeholder actual
...
- **Sí:** solo ROCAS en esta spec. El resto del catálogo queda para specs futuras.
...
- **Sí:** Pausa / Fin / Salir cableados de verdad en ROCAS.
...
- Otros juegos jugables además de ROCAS.
